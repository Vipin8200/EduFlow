    ...
    return (
        <div className="p-4 sm:p-6 max-w-[1700px] mx-auto w-full min-h-screen bg-gray-50/50 font-sans tracking-tight">
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-2xl font-black text-gray-900 tracking-tight text-[#C29543]">Institutional Asset Analytics</h1>
                    <p className="text-sm text-gray-500 font-medium tracking-tight">Generate comprehensive stock audits, consumption trends, and allocation efficiency metrics.</p>
                </div>
            ...
            {/* Inventory Metrics recap */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
                {[
                    { label: 'Avg Consumption', val: '12.4% / Mo', icon: <Zap />, color: 'text-orange-600', bg: 'bg-orange-50/50' },
                    { label: 'Inventory Turnover', val: '4.2x', icon: <TrendingUp />, color: 'text-green-600', bg: 'bg-green-50/50' },
                    { label: 'Low Stock Alerts', val: '8 Items', icon: <AlertCircle />, color: 'text-red-500', bg: 'bg-red-50/50' },
                    { label: 'Registry Integrity', val: 'Verified', icon: <ShieldCheck />, color: 'text-gray-950', bg: 'bg-gray-100' },
                ].map((s, i) => (
                    <div key={i} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm flex items-center justify-between hover:border-[#C29543] transition-all group overflow-hidden relative">
                        ...
                        <div className={`p-4 rounded-[22px] ${s.bg} ${s.color} group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500 shadow-sm shadow-gray-50/50`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                <div className="lg:col-span-2 bg-white p-10 rounded-[50px] border border-gray-100 shadow-xl overflow-hidden relative group border-t-8 border-t-[#C29543]">
                    ...
                    <div className="flex items-center justify-between mb-12">
                        <h3 className="text-xl font-black text-gray-900 italic tracking-tight uppercase underline decoration-orange-100 decoration-8 underline-offset-4">Departmental Consumption Chart</h3>
                        <div className="flex gap-4">
                            <div className="px-4 py-1.5 bg-gray-50 border border-gray-100 rounded-lg text-[10px] font-black text-gray-400 uppercase tracking-widest italic tracking-widest">Asset Usage Volume</div>
                        </div>
                    </div>

                    <div className="h-[250px] flex items-end justify-between gap-6 border-b border-dashed border-gray-100 pb-10">
                        {[65, 42, 85, 92, 55, 78, 60].map((h, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-4 group/bar h-full justify-end">
                                ...
                                <span className="text-[10px] font-black text-gray-400 uppercase italic">Dept {i + 1}</span>
                            </div>
                        ))}
                    </div>
                    <p className="mt-8 text-center text-gray-400 text-xs font-black uppercase tracking-widest italic flex items-center justify-center gap-2"><LayoutGrid size={14} /> Institutionalized Outward Disbursment Balancing</p>
                </div>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <select className="w-full px-5 h-12 bg-gray-50 border border-gray-100 rounded-2xl text-[10px] font-black italic outline-none uppercase tracking-widest">
                                    <option>Stock Level Extraction</option>
                                    <option>Purchase History Audit</option>
                                    <option>Departmental Usage Log</option>
                                    <option>Allocation Metadata</option>
                                </select>
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2 ml-1">Analytical Loop</label>
                ...
                <div className="overflow-x-auto min-h-[300px]">
                    <div className="p-20 text-center flex flex-col items-center gap-6 group">
                        ...
                        <div className="space-y-2">
                            <h4 className="text-2xl font-black text-gray-900 italic tracking-tighter">Analytical Hub Ready</h4>
                            <p className="text-gray-400 text-sm font-medium tracking-tight italic max-w-lg leading-relaxed">Advanced inventory models are synchronized with the primary asset registry. Configure your Analytical Segment above to generate the specialized visualization index.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-16 bg-gray-900 p-12 rounded-[60px] text-white flex flex-col xl:flex-row items-center justify-between gap-12 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-[500px] h-full bg-white/[0.03] rotate-12 -translate-y-20 transition-transform duration-1000 group-hover:translate-x-0"></div>
                <div className="flex-1 space-y-6 relative z-10 text-center xl:text-left">
                    <div className="flex items-center justify-center xl:justify-start gap-4">
                        ...
                        <h3 className="text-2xl font-black italic tracking-widest uppercase underline decoration-[#C29543] decoration-8 underline-offset-4">Asset Forensic Audit Protocols</h3>
                    </div>
                    <p className="text-gray-400 text-sm font-medium leading-relaxed max-w-3xl italic tracking-tight">Institutional inventory reports utilize verifiable data points from inward purchase entries and department-level disbursement logs. All usage summaries are cross-audited for zero-leakage reporting across all campuses.</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-6 relative z-10 w-full xl:w-auto scale-95 md:scale-100 origin-right transition-transform">
                    <button className="flex-1 sm:flex-none px-12 h-14 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-[4px] hover:bg-[#C29543] hover:text-white transition-all shadow-xl shadow-gray-950 italic">Verify Metadata</button>
                    <button className="flex-1 sm:flex-none px-12 h-14 bg-white/10 text-white border border-white/20 rounded-2xl font-black text-[10px] uppercase tracking-[4px] hover:bg-white/20 transition-all backdrop-blur-sm italic">Audit Registry</button>
                </div>
            </div>
        </div>
    );
